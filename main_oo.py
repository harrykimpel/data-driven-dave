...
def load_game_tiles():
    # load all the image files within the directory
    game_tiles = [file for file in listdir(
        "tiles/game/") if isfile(join("tiles/game/", file))]
    ui_tiles = [file for file in listdir(
        "tiles/ui/") if isfile(join("tiles/ui/", file))]
    ...
    for savedfile in game_tiles:
        ...
        tile_name, tile_height, tile_width = graphicPropertiesFromFilename(
            savedfile)

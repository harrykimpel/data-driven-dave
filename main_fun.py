...
# Acquire a tracer
tracer = trace.get_tracer("datadrivendave.tracer")
# Acquire a meter.
meter = metrics.get_meter("gameplay.meter")

# Now create a counter instrument to make measurements with
games_played = meter.create_counter(
    "games_played",
...
def showTitleScreen(screen, tileset, ui_tiles):
    ...
    with tracer.start_as_current_span("showTitleScreen", None, trace.SpanKind.SERVER) as showTitleScreenSpan:
        ...
        while not started_game:
            ...
            screen.printTile(0, BOTTOM_OVERLAY_POS,
                             overlay.getGraphic(ui_tiles))

            # print text in center
            screen.printTextAlignedInCenter(creator_text, 47)
            screen.printTextAlignedInCenter(professor_text, 55)
            screen.printTextAlignedInCenter(
                instr1_text, BOTTOM_OVERLAY_POS + 2)
            screen.printTextAlignedInCenter(
                instr2_text, BOTTOM_OVERLAY_POS + 11)
...
def showInterpic(completed_levels, screen, GamePlayer, tileset, ui_tileset):
    # This creates a new span that's the child of the current one
    with tracer.start_as_current_span("showInterpic", None, trace.SpanKind.SERVER) as showInterpicSpan:
        clock = pygame.time.Clock()
...
def showWarpZone(completed_levels, screen, GamePlayer, tileset, ui_tileset):
    # This creates a new span that's the child of the current one
    with tracer.start_as_current_span("showWarpZone", None, trace.SpanKind.SERVER) as showWarpZoneSpan:
        clock = pygame.time.Clock()
        ...
        # init player
        (player_absolute_x, player_absolute_y) = warp_level.initPlayerPositions(
            0, GamePlayer)
...
def getBonusMapping(current_level):
    # This creates a new span that's the child of the current one
    with tracer.start_as_current_span("getBonusMapping", None, trace.SpanKind.SERVER) as getBonusMappingSpan:
        if current_level == 2:
            return 6
...
def showScores(screen, tileset):
    # This creates a new span that's the child of the current one
    with tracer.start_as_current_span("showScores", None, trace.SpanKind.SERVER) as showScoresSpan:
        pass


def savePlayerScore(player_score, screen, tileset):
    # This creates a new span that's the child of the current one
    with tracer.start_as_current_span("savePlayerScore", None, trace.SpanKind.SERVER) as savePlayerScoreSpan:
        pass


def showCreditsScreen(screen, tileset):
    # This creates a new span that's the child of the current one
    with tracer.start_as_current_span("showCreditsScreen", None, trace.SpanKind.SERVER) as showCreditsScreenSpan:
        pass
...
def main():
    # This creates a new span that's the child of the current one
    with tracer.start_as_current_span("initGame", None, trace.SpanKind.SERVER) as initGameSpan:
        # Init pygame
        pygame.init()
        game_screen = Screen(SCREEN_WIDTH, SCREEN_HEIGHT)

    # This creates a new span that's the child of the current one
    with tracer.start_as_current_span("initTiles", None, trace.SpanKind.SERVER) as initTilesSpan:
        # Init tiles
        tileset, ui_tileset = load_all_tiles()
        game_open = True

    while game_open:
        with tracer.start_as_current_span("gameOpen", None, trace.SpanKind.SERVER) as gameOpenSpan:
            # Show title screen
            option = showTitleScreen(game_screen, tileset, ui_tileset)

            with tracer.start_as_current_span("gamePlay", None, trace.SpanKind.SERVER) as gamePlaySpan:
                ...
                while not ended_game:
                    ...
                    spawner_pos_x = Level.getPlayerSpawnerPosition(current_spawner_id)[
                        0]
                    game_screen.setXPosition(
                        spawner_pos_x - 10, Level.getWidth())
                    ...
                    while not ended_level:
                            ...
                            # hit a key
                                ...
                                if event.key == pygame.K_ESCAPE:
                                    ...
                                    gamePlaySpan.set_attribute(
                                        "player_score", GamePlayer.score)
                                    games_played.add(
                                        1, {"player_score": GamePlayer.score})
                        ...
                        reached_level_left_boundary = (
                            game_screen.getXPosition() <= 0)
                        ...
                        if score_ui != GamePlayer.score:
                            game_screen.updateUiScore(
                                GamePlayer.score, ui_tileset)
                    ...
                    if (player_position_x == -2):
                        current_level_number = getBonusMapping(
                            current_level_number)
